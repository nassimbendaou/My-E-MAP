...
import v1 from "../svg/v1.glb";
import v2 from "../svg/v2.glb";
import v3 from "../svg/v3.glb";
import v4 from "../svg/v4.glb";
import v5 from "../svg/v5.glb";
import t1 from "../svg/t1.glb";
import t2 from "../svg/t2.glb";
...
class MapView3D extends React.Component {
  ...
  show3d = (name) => {
    loadModules(["esri/Graphic", "esri/layers/GraphicsLayer"], {
      ...
      let t = [t1, t2];
      let building = this.state.D3;
      building.forEach((b) => {
        if (b.name === name && b.equipments !== "obj") {
          clr.push(b);
        }
        if (b.equipments === "obj") {
          obj.push(b);
      ...
      clr.forEach((cl) => {
        ...
        let simpleFillSymbol = {
          ...
          symbolLayers: [
            {
              type: "extrude",
              size: cl.rj45_outlets, // extrude by 10 meters
              material: {
                color: [255, 255, 255, 1],
              },
              edges: {
      ...
      obj.forEach((cl) => {
        console.log(cl.latitude[0]);
        console.log(cl.longitude[0]);
        console.log("te 3d");
            ...
            symbolLayers: [
              {
                ...
                height: 6,
                resource: {
                  href: t[Math.floor(Math.random() * 2)],
                },
              },
            ],
          };
            ...
            symbolLayers: [
              {
                ...
                height: 2,
                resource: {
                  href: v[Math.floor(Math.random() * 6)],
                },
              },
            ],
          };
    ...
    xhr.onload = () => {
      ...
      loadModules(
        ...
        ([
          ...
          var cam = new Camera({
            position: {
              x: -5.0055117969942335,
              y: 33.9859292495407,
              z: 499, // meters
            },
            tilt: 86.75560168058132,
            heading: 39.1465084457615,
            fov: 55,
          });
          this.view.goTo(cam).then(() => {
            let arrayLayer = [];
            ...
            this.view.hitTest(evt.screenPoint).then((response) => {
              ...
              if (graphic) {
                if (graphic.attributes.univ === "true") {
                  this.setState({ currentest: graphic.attributes });
                  this.view.environment.lighting.date = new Date(
                    "Sun Mar 15 2019 09:00:00 GMT+0100 (CET)"
                  );
                  ...
                  data.forEach((est) => {
                    if (est.id === graphic.attributes.id) {
                      this.show3d(graphic.attributes.name);
        ...
        <div>
          ...
          <div>
            <div style={{ float: "left" }}>
              <PanelMenu model={this.state.items} style={{ width: "267px" }} />
            </div>
